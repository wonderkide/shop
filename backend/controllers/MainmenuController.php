...
class MainmenuController extends Controller
{
    ...
    public function actionSorting($id) {
        ...
        $nextModel = MainMenu::find()->where($condition)->andWhere(['parent'=>$model->parent])->orderBy($orderBy)->one();
    ...
    private function getSorting($parent) {
        $sort = 1;
        $model = MainMenu::find()->where(['parent' => $parent])->orderBy(['sort' => SORT_DESC])->one();
        if (!empty($model)) {
            $sort +=$model->sort;
        }
        return $sort;
